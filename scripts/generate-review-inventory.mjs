...
const payload = {
  ...
  reviewDimensions: ['answer key', 'difficulty', 'specialization', 'readability', 'language', 'similarity', 'pilot performance'],
  questions,
};
...
for (const q of samples) {
  markdown.push(
    `## ${q.id}`,
    '',
    `${q.competencyLabel} · ${q.difficulty} · ${q.scopeLabel}`,
    '',
    `Recommended live format: **${q.recommendedFormat.format}**`,
    '',
    ...
    q.prompt,
    '',
    ...q.options.map(o => `- **${o.id.toUpperCase()}.** ${o.label}`),
    '',
    `**Answer: ${q.correctOptionIds[0].toUpperCase()}.** ${q.rationale}`,
    '',
  );
}
