...
const generalRelianceQuestions: Question[] = [
  ...
  },
  {
    ...
    stimulus: {
      src: '/stimuli/viral-disaster-post.svg',
      alt: 'Viral disaster social post with dramatic image, unknown source, urgent caption, and missing provenance.',
      label: 'Social post artifact',
...
const competencyDepthQuestionBank: Question[] = [
  ...
  },
  {
    ...
    evidenceMode: 'knowing',
    stimulus: { src: '/stimuli/viral-disaster-post.svg', alt: 'Viral disaster post with dramatic caption, missing date, and repost indicators', label: 'Viral post check', caption: 'The image may be real but the caption may be misleading.' },
  ...
  },
  {
    ...
    evidenceMode: 'doing',
    stimulus: { src: '/stimuli/flooded-street-authenticity.png', alt: 'Flooded street photo used in a questionable breaking-news post', label: 'Image authenticity review', caption: 'The task is to explain uncertainty, not guess from pixels alone.' },
    context: 'A photo is spreading with a claim about a current emergency.',
  ...
  },
  {
    ...
    evidenceMode: 'hybrid',
    stimulus: { src: '/stimuli/viral-disaster-post.svg', alt: 'Viral disaster post with dramatic image, unknown source, urgent caption, and missing provenance', label: 'Viral post response', caption: 'The written answer should combine verification and team communication.' },
    context: 'A team chat is about to repost an urgent disaster image that lacks source, date, and location evidence.',
...
const questionBank: Question[] = [
  ...
  },
  {
    ...
    stimulus: {
      src: '/stimuli/flooded-street-authenticity.png',
      alt: 'A realistic synthetic-looking scene used as a visual provenance exercise.',
...
export default function Home() {
  ...
  function submitAnswer(option: Option, details?: { textResponse?: string; rubricHits?: RubricCriterion[]; partSelections?: Record<string, string>; partScores?: Array<{ partId: string; domain: DomainId; score: number }> }) {
    const completedAnswer = { question: current, option, ...details };
        ...
        <section className="assessment-shell">
          <div className="assessment-workbench">
            <article className="question-card task-card" data-question-id={current.id}>
              ...
              {(current.interaction ?? 'single') === 'single' && current.type !== 'reliance-decision' && (
              ...
              <div className="domain-evidence-grid">
                {domainEvidenceSummary.map((item) => (
                  <button
                    ...
                    onClick={() => setSelectedRadarDomain(item.domain)}
                  >
              ...
              <div className="drilldown-domain-tabs" aria-label="Choose a domain for competency drilldown">
                {(Object.keys(domains) as DomainId[]).map((domain) => (
                  <button
                    ...
                    onClick={() => setSelectedRadarDomain(domain)}
                  >
                    <span>{domain}</span>
